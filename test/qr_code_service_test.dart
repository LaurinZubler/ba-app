...
import 'package:ba_app/domain/model/qrCode/qr_code_model.dart';
...
void main() {
  ...
  group('handleQrCode() - contact', () {
    test('success', () async {
      ...
      final qrCode = QRCode(type: CONTACT_QR_TYPE, data: contact).toJsonString();
  ...
  group('handleQrCode() - invalid QR', () {
    test('contact', () async {
      ...
      final qrCode = const QRCode(type: CONTACT_QR_TYPE, data: data).toJsonString();
      expect(() => qrCodeService.handleQrCode(qrCode, () => noCall(), (p0) => noCall()), throwsA(isA<FormatException>()));
    });

    test('poa', () async {
      const data = {'zero': 0} as Map<String, Object?>;
      final qrCode = const QRCode(type: POA_QR_TYPE, data: data).toJsonString();
      expect(() => qrCodeService.handleQrCode(qrCode, () => noCall(), (p0) => noCall()), throwsA(isA<FormatException>()));
    });
